...
function applyBuffPrefQuick(fairy, nyc) {
  const fEl = document.getElementById('pref-fairy-enabled');
  const nEl = document.getElementById('pref-nyc-enabled');
  if (fEl) fEl.checked = !!fairy;
  if (nEl) nEl.checked = !!nyc;
}

document.getElementById('pref-both-on')?.addEventListener('click', () => applyBuffPrefQuick(true, true));
document.getElementById('pref-fairy-only')?.addEventListener('click', () => applyBuffPrefQuick(true, false));
document.getElementById('pref-nyc-only')?.addEventListener('click', () => applyBuffPrefQuick(false, true));
document.getElementById('pref-both-off')?.addEventListener('click', () => applyBuffPrefQuick(false, false));
...
function renderGarden() {
  ...
  // Trang trí: icon tiên + người chăm vườn (khi buff đang bật)
