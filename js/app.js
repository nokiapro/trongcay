...
function openNycConfigModal() {
  ...
  sel.innerHTML = '<option value="">— Chưa chọn hạt —</option>' + ids.map(id => {
    const p = Game.getPlant(id);
    if (!p) return '';
    const n = (seeds[id] || 0) + (stars[id] || 0);
    return `<option value="${id}" ${cfg.plantId === id ? 'selected' : ''}>${p.icon} ${p.name} (x${n})</option>`;
  }).join('');
  if (cfg.plantId && !ids.includes(cfg.plantId)) {
    const p = Game.getPlant(cfg.plantId);
    if (p) sel.innerHTML += `<option value="${cfg.plantId}" selected>${p.icon} ${p.name} (hết hạt)</option>`;
  }
...
function bindNycConfigUI() {
  ...
  document.getElementById('btn-save-nyc-config')?.addEventListener('click', async () => {
    const plantId = document.getElementById('nyc-plant-select')?.value || null;
    ...
    const res = Game.setNycConfig({ plantId: plantId || null, mode, count });
    if (res.ok) {
      await savePlayer();
      showToast(res.msg, 'success');
      closeModals();
...
function renderGarden() {
  ...
  // Trang trí: cô tiên quanh ô khi có Tiên; người đi quanh khi có NYC
  ...
  document.getElementById('plot-detail').innerHTML = `
    <div class="plot-detail-card">
      ...
      <div class="plot-detail-progress">
        <div class="plot-detail-progress-label">Tiến độ ra hoa/quả: <strong>${Math.min(100, progress)}%</strong></div>
        <div class="plot-progress plot-progress-lg"><div class="plot-progress-bar" style="width:${Math.min(100, progress)}%"></div></div>
      </div>
...
document.getElementById('btn-water-all').addEventListener('click', () => openBulkModal('water'));
document.getElementById('btn-fertilize-all')?.addEventListener('click', () => openBulkModal('fert'));
document.getElementById('btn-harvest-all').addEventListener('click', () => openBulkModal('harvest'));
...
function renderShop() {
  ...
  if (currentShopTab === 'tien') {
    ...
    info.innerHTML = `<i class="fa-solid fa-wand-magic-sparkles"></i> Tiên đang active: <strong>${remain}</strong> — tự tưới khi mưa & gia hạn tưới/phân mỗi 3 giờ`;
  ...
  if (currentShopTab === 'nyc') {
    ...
    DEFAULT_NYC_PACKS.forEach(pack => {
      ...
      card.innerHTML = `
        ...
        <div class="shop-name">${pack.name}</div>
        <span class="shop-type">Buff vườn</span>
        <div class="shop-desc">Tự thu hoạch cây chín + trồng lại ${pack.days} ngày (cộng dồn).</div>
        <div class="shop-price">${pack.price.toLocaleString()} 🪙</div>
...
function renderInventory() {
  ...
  const stars = (currentPlayer.inventory && currentPlayer.inventory.seedsStar) || {};
  const seedIds = [...new Set([...Object.keys(seeds), ...Object.keys(stars)])].filter(id => (seeds[id] || 0) + (stars[id] || 0) > 0);
  if (seedIds.length === 0) {
    ...
    seedsEl.innerHTML = '<div class="inv-grid">' + seedIds.map(id => {
      ...
      const n = seeds[id] || 0;
      const s = stars[id] || 0;
      const unitN = Math.max(1, Math.floor((plant.seedPrice || 1) * 0.5));
      const unitS = Math.max(1, Math.floor((plant.seedPrice || 1) * 0.75));
      return `
        <div class="inv-item">
          <div class="icon">${plant.icon}</div>
          <div class="name">${plant.name}</div>
          <div class="qty">Thường x${n.toLocaleString()}${s ? ' · ⭐ x' + s : ''}</div>
          <div class="qty sell-hint">${unitN}🪙/thường${s ? ' · ' + unitS + '🪙/⭐' : ''}</div>
          <div class="actions">
            ${n > 0 ? `<button class="btn btn-success btn-sell-seed" data-id="${id}" data-kind="normal" data-qty="1">Bán 1</button>
            <button class="btn btn-primary btn-sell-seed" data-id="${id}" data-kind="normal" data-qty="all">Bán hết thường</button>` : ''}
            ${s > 0 ? `<button class="btn btn-warning btn-sell-seed" data-id="${id}" data-kind="star" data-qty="all">Bán hết ⭐</button>` : ''}
            ${(n > 0 && s > 0) ? `<button class="btn btn-secondary btn-sell-seed" data-id="${id}" data-kind="all" data-qty="all">Bán tất cả</button>` : ''}
          </div>
        </div>
      `;
    }).join('') + '</div>';
...
function softUpdateGarden() {
  ...
  if (typeof Game.resetExpiredBoosts === 'function') {
    const changed = Game.resetExpiredBoosts();
    if (changed && typeof savePlayer === 'function') savePlayer().catch(() => {});
  }
  if (typeof Game.tickNycCare === 'function' && Game.hasNyc()) {
    Game.tickNycCare().then(did => {
      if (did && typeof savePlayer === 'function') {
        savePlayer().catch(() => {});
