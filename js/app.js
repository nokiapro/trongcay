...
function renderShop() {
  ...
  if (currentShopTab === 'tangtoc') {
    ...
    const plotOpts = (currentPlayer && currentPlayer.plots ? currentPlayer.plots : []).map((pl, i) => {
      ...
      return `<option value="${i}">Ô #${i + 1}${pl.plantId ? ' · đang trồng' : ' · trống'} · x${sm}</option>`;
    }).join('');
    packs.forEach(pack => {
      ...
      card.innerHTML = `
        ...
        <select class="boost-plot-sel" data-id="${pack.id}" style="width:100%;margin:6px 0">${plotOpts || '<option value="">Chưa có ô</option>'}</select>
...
function bindBankAmountStepper() {
  const inp = document.getElementById('bank-amount');
  const minus = document.getElementById('bank-amt-minus');
  const plus = document.getElementById('bank-amt-plus');
  if (!inp || !minus || !plus) return;
  const clamp = (v) => Math.max(100, Math.min(999999999, v));
  minus.onclick = () => { inp.value = clamp((parseInt(inp.value, 10) || 500) - 100); };
  plus.onclick = () => { inp.value = clamp((parseInt(inp.value, 10) || 500) + 100); };
}
bindBankAmountStepper();
...
/** Bọc input số lượng thành nút − / + đẹp */
function enhanceQtyInputs(root) {
  const scope = root || document;
  scope.querySelectorAll('input.qty-input, input.plant-qty-input').forEach(inp => {
    ...
    minus.addEventListener('click', e => {
      ...
      e.stopPropagation();
      const s = step();
      inp.value = Math.max(s.min, s.v - 1);
    ...
    plus.addEventListener('click', e => {
      ...
      e.stopPropagation();
      const s = step();
      inp.value = Math.min(s.max, s.v + 1);
      inp.dispatchEvent(new Event('change', { bubbles: true }));
    });
  });
}
