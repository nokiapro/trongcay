...
const Features = {
  ...
  async upgradePlot(plotId, targetMult) {
    ...
    if ((currentPlayer.coins || 0) < cost) return { ok: false, msg: 'Không đủ xu! Cần ' + cost.toLocaleString() + '<i class="fa-solid fa-coins"></i>' };
    ...
    if (typeof Game !== 'undefined' && Game.addActivity) {
      Game.addActivity('Nâng ô #' + (plotId + 1) + ' → x' + tgt + ' (-' + cost + '<i class="fa-solid fa-coins"></i>)');
    }
    await savePlayer();
  ...
  async upgradeAllPlotsTo(targetMult = 50) {
    ...
    if ((currentPlayer.coins || 0) < totalCost) {
      return {
        ok: false,
        msg: 'Không đủ xu! Cần ' + totalCost.toLocaleString() + '<i class="fa-solid fa-coins"></i> để nâng ' + jobs.length + ' ô → x' + tgt,
        need: totalCost,
        count: jobs.length
      };
    }
    ...
    if (typeof Game !== 'undefined' && Game.addActivity) {
      Game.addActivity('Nâng ' + jobs.length + ' ô (mọi vườn) → x' + tgt + ' vĩnh viễn (-' + totalCost.toLocaleString() + '<i class="fa-solid fa-coins"></i>)');
    }
    await savePlayer();
    if (typeof updateCoins === 'function') updateCoins();
    return {
      ok: true,
      msg: 'Đã nâng ' + jobs.length + ' ô → x' + tgt + ' vĩnh viễn (-' + totalCost.toLocaleString() + '<i class="fa-solid fa-coins"></i>)!',
  ...
  async buyTempPlotBoost(packId, plotId) {
    ...
    if (typeof Game !== 'undefined' && Game.addActivity) {
      Game.addActivity('Boost ô #' + (plotId + 1) + ' ' + pack.name + ' (-' + pack.price + '<i class="fa-solid fa-coins"></i>)');
    }
    await savePlayer();
    if (typeof updateCoins === 'function') updateCoins();
  ...
  async claimQuest(scope, id) {
    ...
    if (typeof Game !== 'undefined' && Game.addActivity) {
      Game.addActivity(`Nhiệm vụ ${scope === 'weekly' ? 'tuần' : 'ngày'}: ${def.title} +${def.reward}<i class="fa-solid fa-coins"></i>`);
    }
    await savePlayer();
    if (typeof updateCoins === 'function') updateCoins();
    return { ok: true, msg: `Nhận ${def.reward}<i class="fa-solid fa-coins"></i> + ${def.xp || 0} XP!` };
  },
  ...
  async bankDeposit(amount, termId) {
    ...
    if (typeof Game !== 'undefined' && Game.addActivity) {
      Game.addActivity(`Gửi ngân hàng ${amount}<i class="fa-solid fa-coins"></i> · ${term.label}`);
    }
    await savePlayer();
    if (typeof updateCoins === 'function') updateCoins();
    return { ok: true, msg: `Đã gửi ${amount}<i class="fa-solid fa-coins"></i> (${term.label}, lãi ${Math.round(term.rate * 100)}%)` };
  },
  ...
  async bankTopUp(depId, amount) {
    ...
    if (typeof Game !== 'undefined' && Game.addActivity) {
      Game.addActivity(`Nạp thêm ngân hàng +${amount}<i class="fa-solid fa-coins"></i> → gốc ${d.amount}<i class="fa-solid fa-coins"></i>`);
    }
    await savePlayer();
    if (typeof updateCoins === 'function') updateCoins();
    return { ok: true, msg: `Đã nạp thêm ${amount}<i class="fa-solid fa-coins"></i>. Gốc hiện tại: ${d.amount.toLocaleString()}<i class="fa-solid fa-coins"></i>` };
  },
  ...
  async bankWithdraw(depId, mode = 'all') {
    ...
    if (mode === 'interest') {
      ...
      if (typeof Game !== 'undefined' && Game.addActivity) {
        Game.addActivity(`Rút lãi ngân hàng +${interestPay}<i class="fa-solid fa-coins"></i> (gốc ${principal.toLocaleString()}<i class="fa-solid fa-coins"></i> tiếp tục gửi)`);
      }
      await savePlayer();
      if (typeof updateCoins === 'function') updateCoins();
      return { ok: true, msg: `Đã rút lãi +${interestPay.toLocaleString()}<i class="fa-solid fa-coins"></i>. Gốc ${principal.toLocaleString()}<i class="fa-solid fa-coins"></i> tiếp tục sinh lãi!` };
    }
    ...
    if (typeof Game !== 'undefined' && Game.addActivity) {
      Game.addActivity(`Rút ngân hàng +${payout.toLocaleString()}<i class="fa-solid fa-coins"></i> (gốc ${principal.toLocaleString()} + lãi ${interestPay.toLocaleString()})`);
    }
    await savePlayer();
    if (typeof updateCoins === 'function') updateCoins();
    return { ok: true, msg: `Đã rút +${payout.toLocaleString()}<i class="fa-solid fa-coins"></i> (gốc ${principal.toLocaleString()} + lãi ${interestPay.toLocaleString()})!` };
  },
  ...
  async redeemGiftCode(code) {
    ...
    if (coins) parts.push('+' + coins + '<i class="fa-solid fa-coins"></i>');
    if (plotsAdd > 0) {
  ...
  async buyMarketItem(listingId) {
    ...
    const payNote = `Chợ: bán ${L.qty} ${L.itemName || L.itemId} +${cost}<i class="fa-solid fa-coins"></i> (từ ${buyerName})`;
    try {
    ...
    this.trackQuest('market', 1);
    if (typeof Game !== 'undefined' && Game.addActivity) {
      Game.addActivity(`Chợ: mua ${L.qty} ${L.itemName} -${cost}<i class="fa-solid fa-coins"></i>`);
    }
    await savePlayer();
    if (typeof updateCoins === 'function') updateCoins();
  ...
  async claimMarketCredits() {
    ...
    try {
      ...
      for (const id of ids) {
        ...
        const note = c.note || `Chợ: bán hàng +${amount}<i class="fa-solid fa-coins"></i>`;
