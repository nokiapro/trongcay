...
const Game = {
  ...
  // Stage thresholds in seconds (absolute along effective timeline)
  getStageThresholds(plot) {
    const total = this.getEffectiveGrowTime(plot);
    // Hạt → cây non ≥ 60s
    const t1 = Math.max(60, total * 0.22);           // cây non
    const t2 = Math.max(t1 + 30, total * 0.45);      // cây trung
    const t3 = Math.max(t2 + 30, total * 0.70);      // trưởng thành
    const t4 = total;                                 // ra hoa/quả (ready)
    return [t1, t2, t3, t4];
  },
  ...
  getStage(plot) {
    ...
    if (ready) {
      return { key: 'ready', icon: plant ? plant.icon : '✨', label: 'Ra hoa/quả', idx: 4 };
    }
    if (elapsed >= t3) {
      return { key: 'mature', icon: '🌳', label: 'Trưởng thành', idx: 3 };
    }
    if (elapsed >= t2) {
      return { key: 'young', icon: '🌿', label: 'Cây trung', idx: 2 };
    }
    if (elapsed >= t1) {
      return { key: 'seedling', icon: '🌱', label: 'Cây non', idx: 1 };
    }
