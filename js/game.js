...
const Game = {
  ...
  async simulateOfflineCare() {
    ...
    else if (lastCatch > 0) leaveAt = lastCatch;
    else leaveAt = now;
...
let changed = false;
        ...
        for (let gi = 0; gi < (currentPlayer.gardens || []).length; gi++) {
          try {
            ...
            for (let i = 0; i < plots.length; i++) {
              ...
              // Giới hạn theo thời gian offline thật (không cho hàng trăm vòng khi off ngắn)
    ...
    if (this.isHelperActive()) {
      // Giúp việc: cooldown 12s — chỉ bù đúng số lần trong cửa sổ offline
      const prev = currentPlayer.lastHelperBuy || 0;
      const HELPER_CD = 12000;
      let buys = 0;
      let helperItems = 0;
      let tBuy = Math.max((prev > 0 ? prev + HELPER_CD : from), from);
      const maxBuys = Math.min(300, Math.max(0, Math.floor(offlineGap / HELPER_CD) + 1));
      let guard = 0;
      while (tBuy <= now && buys < maxBuys && guard++ < maxBuys + 2) {
        currentPlayer.lastHelperBuy = Math.max(0, tBuy - HELPER_CD);
        const n = this.tickHelperBuy(tBuy) || 0;
        if (n > 0) {
          buys++;
          helperItems += n;
          changed = true;
    ...
    if (this.isRobotActive && this.isRobotActive()) {
      try {
        ...
        // Off rất ngắn (<2 phút): không cho robot mua/ghép bulk (tránh log "làm cả núi")
        if (offlineGap < 2 * 60 * 1000) {
          // bỏ qua robot offline
        } else if (Object.keys(fairyMap).length) {
    ...
    if (shouldLog) {
      try {
        ...
        // Không đổ số offline vào sessionStats online (tránh log online bị phình)
        // Offline đã nằm trong logOfflineReport riêng.
      } catch (logErr) {
  ...
  tickHelperBuy(now = (typeof nowMs==="function"?nowMs():Date.now())) {
    ...
    if (totalBought > 0) {
      ...
      this.trackDayStat('helper_buy', { qty: lines.length, cost: totalCost, lines: lines.slice() });
      this.addActivity(name + ' mua: ' + lines.slice(0, 5).join(', ') + ' (-' + totalCost.toLocaleString() + ' xu)', { type: 'helper_buy' });
    }
    return totalBought;
  },
  ...
  async claimDaily() {
    ...
    try {
      ...
      let msg = 'Streak ' + streak + ' ngày 🔥 · Nhận ' + reward.toLocaleString() + ' xu';
  ...
  /** Stats rỗng cho 1 phiên log (online) */
  _emptySessionStats(sessionId, dayKey, startedAt) {
    return {
      sessionId: sessionId,
      dayKey: dayKey,
      startedAt: startedAt || ((typeof nowMs === 'function') ? nowMs() : Date.now()),
      gardens: {},
      fairy: { gardensWatered: 0, waterActions: 0, rainSeeds: 0, fertActions: 0, _gSet: {} },
      nyc: { gardens: 0, plots: 0, harvestYield: 0, byGarden: {}, _gSet: {} },
      helper: { fertBought: 0, spent: 0, items: {} },
  ...
  /**
   * Phiên log online hiện tại.
   * Mỗi lần offline xong / sang ngày mới → phiên mới, số đếm về 0.
   * Không gộp cả 24h vào một khối.
   */
  ensureLogSession(kind) {
    if (!currentPlayer) return null;
    const k = kind || 'online';
    const dayKey = this._dayKey();
    const now = (typeof nowMs === 'function') ? nowMs() : Date.now();
    let s = currentPlayer.logSession;
    const needNew = !s || s.kind !== k || (k === 'online' && s.dayKey !== dayKey);
    if (needNew) {
      // Chốt log tổng hợp phiên cũ trước khi sang phiên mới
      if (s && s.kind === 'online' && currentPlayer.sessionStats) {
        try { this.syncAggregatedDayLogs('session_end'); } catch (_) {}
      }
      const id = 'sess_' + now.toString(36) + '_' + Math.random().toString(36).slice(2, 6);
      currentPlayer.logSession = { id: id, kind: k, dayKey: dayKey, startedAt: now };
      if (k === 'online') {
        currentPlayer.sessionStats = this._emptySessionStats(id, dayKey, now);
      }
    }
    if (k === 'online') {
      if (!currentPlayer.sessionStats || currentPlayer.sessionStats.sessionId !== currentPlayer.logSession.id) {
        currentPlayer.sessionStats = this._emptySessionStats(currentPlayer.logSession.id, dayKey, currentPlayer.logSession.startedAt || now);
      }
    }
    return currentPlayer.logSession;
  },

  /** Bắt đầu phiên online mới (sau offline) — số NYC/Robot/Tiên/Giúp việc đếm lại từ 0 */
  startFreshOnlineSession() {
    ...
    try { this.syncAggregatedDayLogs('session_end'); } catch (_) {}
    const id = 'sess_' + now.toString(36) + '_' + Math.random().toString(36).slice(2, 6);
    currentPlayer.logSession = { id: id, kind: 'online', dayKey: dayKey, startedAt: now };
    currentPlayer.sessionStats = this._emptySessionStats(id, dayKey, now);
  },

  ensureSessionStats() {
    this.ensureLogSession('online');
    return currentPlayer.sessionStats || null;
  },

  _gardenSessionStat(gi) {
    const ss = this.ensureSessionStats();
    if (!ss) return null;
  ...
  formatEventResultLine(ev) {
    ...
    if (coins != null && Number(coins) !== 0) {
      ...
      parts.push((n > 0 ? '+' : '') + n.toLocaleString() + ' xu');
    }
    if (cost != null && Number(cost) > 0) parts.push('−' + Number(cost).toLocaleString() + ' xu');
    // Offline summary
  ...
  formatEventSummaryText(ev) {
    ...
    if (act === 'cook' || act === 'robot_cook') {
      return 'Robot nau ' + (q != null ? q + ' × ' : '') + (tName || 'món');
    }
    if (act === 'merge' || act === 'robot_merge') {
      const star = (ev.detail && ev.detail.star) || 0;
      const myth = (ev.detail && ev.detail.myth) || 0;
      if (star || myth) return 'Robot ghep ⭐×' + star + (myth ? ' · ✨×' + myth : '');
      return 'Robot ghep ' + (tName || 'vật phẩm') + (q != null ? ' ×' + q : '');
    }
    if (act === 'plant' || act === 'replant') {
      return 'Trong ' + (tName || 'cây') + cell + (act === 'replant' ? ' (lại)' : '');
    }
    if (act === 'water') return 'Tuoi ' + (tName || 'cây') + cell;
    if (act === 'fert' || act === 'fertilize') return 'Bon ' + (tName || 'phân') + cell;
    if (act === 'harvest') {
      let s = 'Thu hoach ' + (tName || 'cây') + cell;
      if (q != null) s += ' · ×' + q;
      return s;
    }
    if (act === 'crop_ready') return (tName || 'Cay') + cell + ' đã chín';
    if (act === 'remove' || act === 'uproot') return 'Nho ' + (tName || 'cây') + cell;
    if (act === 'level_up' || act === 'levelup') {
      ...
      if (from != null && to != null) return 'Len cap Lv ' + from + ' → Lv ' + to;
      return 'Len cap';
    }
    ...
    if (act === 'reward' || act === 'daily') {
      return 'Nhan thuong' + (ev.coins ? (' · +' + Number(ev.coins).toLocaleString() + ' xu') : '');
    }
    if (act === 'rain' || act === 'rain_start') return 'Mua';
    if (act === 'offline_end' || act === 'offline') {
      const d = ev.detail || {};
      const dur = d.durationText || '';
      return 'Offline' + (dur ? (' · ' + dur) : '');
    }
    if (act === 'offline_start') return 'Bat dau offline';
    if (act === 'fairy_water') return 'Tien tuoi' + cell + (tName ? (' · ' + tName) : '');
    if (act === 'fairy_rain_seed' || act === 'fairy_care') {
      if (raw) return raw.indexOf('🧚') === 0 ? raw : ('🧚 ' + raw);
      return 'Tien cham vuon';
    }
    if (act === 'nyc_harvest' || (ev.actor === 'nyc' && act === 'harvest')) {
      return 'NYC thu hoach' + (tName ? (' ' + tName) : '') + (q != null ? (' ×' + q) : '');
    }
    if (act === 'nyc_plant' || (ev.actor === 'nyc' && act === 'plant')) {
      return 'NYC trong' + (tName ? (' ' + tName) : '') + cell;
    }
    if (act === 'helper_buy') return 'Giup viec mua' + (tName ? (' ' + tName) : '') + (q != null ? (' ×' + q) : '');

    // String thô từ game — làm sạch nhẹ
    if (raw) {
      // NYC / Robot đã có trong text
      if (/^NYC\b/i.test(raw)) return raw;
      if (/^Robot\b/i.test(raw)) return raw;
      return raw;
    }
  ...
  trackDayStat(kind, data) {
    ...
    const ds = this.ensureDayStats();
    if (!ds) return;
    // Phiên online hiện tại — không gộp với offline / phiên trước
    const ss = this.ensureSessionStats();
    const d = data || {};
    ...
    switch (String(kind)) {
      ...
      case 'helper_buy': {
        ...
        if (!ds.helper.items) ds.helper.items = {};
        if (d.items && typeof d.items === 'object') {
          Object.keys(d.items).forEach(nm => {
            ds.helper.items[nm] = (ds.helper.items[nm] || 0) + (Number(d.items[nm]) || 0);
          });
        } else if (d.name) {
          ds.helper.items[d.name] = (ds.helper.items[d.name] || 0) + (Number(d.qty) || 1);
        } else if (Array.isArray(d.lines)) {
          d.lines.forEach(ln => {
            const s = String(ln || '');
            if (!s) return;
            ds.helper.items[s] = (ds.helper.items[s] || 0) + 1;
          });
        }
        break;
      }
    ...
    // Nhân bản vào sessionStats (phiên online hiện tại)
    try { this._mirrorSessionStat(kind, d, gi); } catch (_) {}

    try { this._pushDayEvent(kind, d, gi); } catch (_) {}

    // Auto-push / cập nhật log tổng hợp PHIÊN
    try {
      this._autoPushFromTrack(kind, d, gi);
    } catch (_) {}
  },

  /** Ghi vào sessionStats — cùng logic dayStats nhưng theo phiên */
  _mirrorSessionStat(kind, d, gi) {
    const ss = this.ensureSessionStats();
    if (!ss) return;
    d = d || {};
    const k = String(kind);
    switch (k) {
      case 'plant': {
        const g = this._gardenSessionStat(gi);
        if (!g) break;
        const n = Math.max(0, Number(d.plots) || Number(d.qty) || 1);
        g.plantedQty += n;
        g.plantActions += Math.max(1, Number(d.actions) || 1);
      ...
      case 'fairy_water': {
        ss.fairy.waterActions += Math.max(0, Number(d.actions) || Number(d.plots) || 1);
        ss.fairy._gSet = ss.fairy._gSet || {};
        if (d.gardenIndex != null) {
          ss.fairy._gSet[String(d.gardenIndex)] = 1;
          ss.fairy.gardensWatered = Object.keys(ss.fairy._gSet).length;
        }
        break;
      }
      case 'fairy_fert': {
        ss.fairy.fertActions += Math.max(0, Number(d.actions) || Number(d.plots) || 1);
        break;
      }
      case 'fairy_rain_seed': {
        const q = Math.max(0, Number(d.qty) || 1);
        ss.fairy.rainSeeds += q;
        ss.rainSeeds = (ss.rainSeeds || 0) + q;
        break;
      }
      case 'nyc_plant': {
        const plots = Math.max(0, Number(d.plots) || 1);
        ss.nyc.plots += plots;
        ss.nyc._gSet = ss.nyc._gSet || {};
        const gk = String(d.gardenIndex != null ? d.gardenIndex : gi);
        ss.nyc._gSet[gk] = 1;
        ss.nyc.gardens = Object.keys(ss.nyc._gSet).length;
        if (!ss.nyc.byGarden[gk]) ss.nyc.byGarden[gk] = { plots: 0, yield: 0 };
        ss.nyc.byGarden[gk].plots = (ss.nyc.byGarden[gk].plots || 0) + plots;
        break;
      }
      case 'nyc_harvest': {
        const y = Math.max(0, Number(d.yield) || Number(d.qty) || 0);
        const plots = Math.max(0, Number(d.plots) || 1);
        ss.nyc.harvestYield += y;
        ss.nyc.plots += plots;
        ss.nyc._gSet = ss.nyc._gSet || {};
        const gk = String(d.gardenIndex != null ? d.gardenIndex : gi);
        ss.nyc._gSet[gk] = 1;
        ss.nyc.gardens = Object.keys(ss.nyc._gSet).length;
        if (!ss.nyc.byGarden[gk]) ss.nyc.byGarden[gk] = { plots: 0, yield: 0 };
        ss.nyc.byGarden[gk].yield = (ss.nyc.byGarden[gk].yield || 0) + y;
        ss.nyc.byGarden[gk].plots = (ss.nyc.byGarden[gk].plots || 0) + plots;
        break;
      }
      case 'helper_buy': {
        ss.helper.fertBought += Math.max(0, Number(d.qty) || 1);
        ss.helper.spent += Math.max(0, Number(d.cost) || 0);
        if (!ss.helper.items) ss.helper.items = {};
        if (d.items && typeof d.items === 'object') {
          Object.keys(d.items).forEach(nm => {
            ss.helper.items[nm] = (ss.helper.items[nm] || 0) + (Number(d.items[nm]) || 0);
          });
        } else if (d.name) {
          ss.helper.items[d.name] = (ss.helper.items[d.name] || 0) + (Number(d.qty) || 1);
        } else if (Array.isArray(d.lines)) {
          d.lines.forEach(ln => {
            const s = String(ln || '');
            if (s) ss.helper.items[s] = (ss.helper.items[s] || 0) + 1;
          });
        }
        break;
      }
      case 'robot_seed': {
        const nm = d.name || d.seedName || 'hạt';
        const q = Math.max(0, Number(d.qty) || 1);
        ss.robot.seedsBought[nm] = (ss.robot.seedsBought[nm] || 0) + q;
        ss.robot.seedCost += Math.max(0, Number(d.cost) || 0);
        break;
      }
      case 'robot_cook': {
        const nm = d.name || 'món';
        const q = Math.max(0, Number(d.qty) || 1);
        ss.robot.cooked[nm] = (ss.robot.cooked[nm] || 0) + q;
        ss.robot.cookCount += q;
        break;
      }
      case 'robot_merge': {
        ss.robot.mergeStar += Math.max(0, Number(d.star) || 0);
        ss.robot.mergeMyth += Math.max(0, Number(d.myth) || 0);
        break;
      }
      case 'rain': {
        ss.rainCount += Math.max(0, Number(d.count) || 1);
        break;
      }
      default:
        break;
  ...
  /** Sau trackDayStat → cập nhật log TỔNG HỢP theo actor (1 dòng/ngày/actor) */
  _autoPushFromTrack(kind, d, gi) {
    const k = String(kind || '');
    // Level / thưởng: vẫn 1 event riêng (hiếm)
    if (k === 'levelup' || k === 'level_up') {
      const lv = (d && d.level) || (currentPlayer && currentPlayer.level);
      this.pushGameEvent({
        action: 'level_up',
        actor: 'system',
        category: 'level',
        aggregated: false,
        summaryText: 'Lên cấp Lv ' + lv,
        detail: { level: lv, from: (d && d.from) || null, to: lv },
        result: {}
      });
      return;
    }
    if (k === 'daily' || k === 'reward') {
      this.pushGameEvent({
        ...
        category: 'reward',
        aggregated: false,
        summaryText: 'Nhận thưởng' + (d && d.coins ? (' · +' + Number(d.coins).toLocaleString() + ' xu') : ''),
        coins: d && d.coins,
        detail: Object.assign({}, d || {}),
        result: { currency: (d && d.coins) || 0 }
      });
      return;
    }
  ...
  /**
   * 1 log/ngày cho mỗi actor: nyc | robot | fairy | helper | garden
  ...
  syncAggregatedDayLogs(triggerKind) {
    ...
    const ss = this.ensureSessionStats();
    if (!sess || !ss) return;
    const dayKey = sess.dayKey || this._dayKey();
    const sessionId = sess.id;
    const now = (typeof nowMs === 'function') ? nowMs() : Date.now();
    const list = this.ensureGameEvents();
    // Nguồn số liệu = sessionStats (không phải dayStats cả ngày)
    const ds = ss;

    const upsert = (actor, payload) => {
      if (!payload || !payload.summaryText) return;
      // ID theo phiên — offline xong / ngày mới = log mới, không đè phiên cũ
      const id = 'agg_' + String(sessionId).replace(/\W+/g, '_') + '_' + actor;
      const existing = list.find(e => e && e.id === id);
      const firstAt = (existing && existing.firstAt) ? existing.firstAt : (ss.startedAt || sess.startedAt || now);
      const lastAt = now;
      const detail = Object.assign({}, payload.detail || {}, {
        firstAt: firstAt,
        lastAt: lastAt,
        firstClock: this.formatLogClock(firstAt, true),
        lastClock: this.formatLogClock(lastAt, true),
        timeRange: this.formatLogClock(firstAt, false) + ' – ' + this.formatLogClock(lastAt, false)
      });
      // HH:mm – HH:mm (hoặc 1 mốc nếu mới bắt đầu)
      let timeText = this.formatLogClock(lastAt, true);
      if (firstAt && lastAt && Math.abs(lastAt - firstAt) >= 60000) {
        timeText = this.formatLogClock(firstAt, false) + ' – ' + this.formatLogClock(lastAt, false);
      }
      const base = {
        id: id,
        action: 'day_summary',
        actor: actor,
        category: actor === 'garden' ? 'garden' : actor,
        filter: actor === 'garden' ? 'garden' : actor,
        mode: 'online',
        aggregated: true,
        dayKey: dayKey,
        timestamp: lastAt,
        ...
        lastAt: lastAt,
        timeText: timeText,
        summaryText: this.stripLogEmoji(payload.summaryText),
        text: this.stripLogEmoji(payload.summaryText),
        resultLine: this.stripLogEmoji(payload.resultLine || ''),
        detail: detail,
      ...
      if (existing) {
        ...
        // giữ firstAt gốc
        existing.firstAt = firstAt;
        existing.detail = existing.detail || {};
        existing.detail.firstAt = firstAt;
        existing.detail.firstClock = this.formatLogClock(firstAt, true);
      } else {
        list.unshift(base);
      }
    };
    ...
    // —— NYC ——
    const nyc = ds.nyc || {};
    if ((nyc.plots || 0) > 0 || (nyc.harvestYield || 0) > 0 || (nyc.gardens || 0) > 0) {
      const byG = nyc.byGarden || {};
      const gardenLines = Object.keys(byG).map(gk => {
        const g = byG[gk] || {};
        return {
          gardenIndex: Number(gk),
          gardenLabel: 'Vườn ' + (Number(gk) + 1),
          plantTimes: Number(g.plots) || 0, // moi o = 1 lan
          yield: Number(g.yield) || 0
        };
      });
      const totalPlant = gardenLines.reduce((s, x) => s + x.plantTimes, 0) || (nyc.plots || 0);
      const totalYield = gardenLines.reduce((s, x) => s + x.yield, 0) || (nyc.harvestYield || 0);
      ...
      upsert('nyc', {
        ...
        detail: {
          ...
          name: name,
          gardens: nGardens,
          plantTimes: totalPlant,
          harvestYield: totalYield,
          byGarden: gardenLines,
          note: 'Mỗi ô tính 1 lần trồng/thu'
        },
        result: { sp: totalYield }
      });
    }

    // —— Robot ——
    const rb = ds.robot || {};
    const seeds = rb.seedsBought || {};
    const cooked = rb.cooked || {};
    const seedEntries = Object.keys(seeds).map(nm => ({ name: nm, qty: seeds[nm] }));
    const cookEntries = Object.keys(cooked).map(nm => ({ name: nm, qty: cooked[nm] }));
    const hasRobot = seedEntries.length || cookEntries.length || (rb.mergeStar || 0) || (rb.mergeMyth || 0) || (rb.seedCost || 0);
    if (hasRobot) {
      const rname = (this.getRobotDisplayName && this.getRobotDisplayName()) || 'Robot';
      const parts = [];
      const seedQty = seedEntries.reduce((s, x) => s + (x.qty || 0), 0);
      if (seedQty) parts.push('mua ' + seedQty.toLocaleString() + ' hạt');
      if (rb.cookCount) parts.push('nấu ' + rb.cookCount.toLocaleString() + ' món');
      if (rb.mergeStar) parts.push('ghép sao ×' + Number(rb.mergeStar).toLocaleString());
      ...
      upsert('robot', {
        ...
        detail: {
          actor: 'robot',
          name: rname,
          seedsBought: seedEntries,
          seedCost: cost,
          cooked: cookEntries,
          cookCount: rb.cookCount || 0,
          mergeStar: rb.mergeStar || 0,
          mergeMyth: rb.mergeMyth || 0
        },
        result: { currency: cost ? -cost : 0 }
      });
    }

    // —— Fairy ——
    const fy = ds.fairy || {};
    if ((fy.waterActions || 0) || (fy.fertActions || 0) || (fy.rainSeeds || 0) || (fy.gardensWatered || 0)) {
      const fname = (this.getFairyDisplayName && this.getFairyDisplayName()) || 'Tiên';
      const parts = [];
      if (fy.gardensWatered) parts.push(fy.gardensWatered + ' vuon');
      if (fy.waterActions) parts.push('tưới ' + fy.waterActions.toLocaleString() + ' lan');
      ...
      upsert('fairy', {
        ...
        detail: {
          actor: 'fairy',
          name: fname,
          gardensWatered: fy.gardensWatered || 0,
          waterActions: fy.waterActions || 0,
          fertActions: fy.fertActions || 0,
          rainSeeds: fy.rainSeeds || 0
        },
        result: {}
      });
    }
    ...
    // —— Helper ——
    const hp = ds.helper || {};
    if ((hp.fertBought || 0) || (hp.spent || 0) || (hp.items && Object.keys(hp.items).length)) {
      const hname = (this.getHelperDisplayName && this.getHelperDisplayName()) || 'Giúp việc';
      ...
      upsert('helper', {
        ...
        result: { currency: hp.spent ? -hp.spent : 0 }
      });
    }

    // —— Vườn (người chơi) ——
    const gardens = ds.gardens || {};
    let pPlant = 0, pHarvest = 0, pYield = 0, pReplant = 0;
    const gLines = [];
    Object.keys(gardens).forEach(gk => {
      const gg = gardens[gk] || {};
      const planted = (gg.plantedQty || 0) + (gg.replanted || 0);
      const hy = gg.harvestYield || 0;
      const ph = gg.plotsHarvested || 0;
      if (planted || hy || ph) {
        pPlant += planted;
        pHarvest += ph;
        pYield += hy;
        pReplant += gg.replanted || 0;
        gLines.push({
          gardenIndex: Number(gk),
          gardenLabel: 'Vườn ' + (Number(gk) + 1),
          planted: planted,
          plotsHarvested: ph,
          yield: hy,
          replanted: gg.replanted || 0
        });
      }
    });
    if (pPlant || pHarvest || pYield) {
      const parts = [];
      if (pPlant) parts.push('trồng ' + pPlant + ' lần');
  ...
  addOfflineLog(report) {
    ...
    this.pushGameEvent({
      ...
      firstAt: startedAt,
      lastAt: now,
      timeText: this.formatLogClock(startedAt, false) + ' – ' + this.formatLogClock(now, false),
      summaryText: 'Offline · ' + duration,
      detail: entry.detail,
      result: {
        sp: garden.product || 0,
        xp: xp || 0
    ...
    // Chốt phiên online cũ + bắt đầu phiên online MỚI (số đếm về 0)
    try { this.startFreshOnlineSession(); } catch (_) {}
  ...
  getActivityLogList() {
    ...
    // Các action rời rạc cần ẩn (đã gộp vào tổng hợp)
    const HIDE = {
      plant: 1, replant: 1, water: 1, fert: 1, fertilize: 1, harvest: 1, crop_ready: 1,
      remove: 1, uproot: 1, xp: 1, xp_gain: 1,
      robot_seed: 1, robot_cook: 1, robot_merge: 1, robot_rain: 1,
      fairy_water: 1, fairy_fert: 1, fairy_rain_seed: 1, fairy_care: 1,
      nyc_plant: 1, nyc_harvest: 1, helper_buy: 1, buy_seed: 1
    };
    ...
    events.forEach(e => {
      ...
      // Giữ: aggregated, offline, level_up, reward, day_summary, và string addActivity quan trọng
      if (e.aggregated || act === 'day_summary') {
        // ok
      } else if (act === 'offline_end' || act === 'offline' || e.mode === 'offline' || e.category === 'offline') {
        // ok
      } else if (act === 'level_up' || act === 'levelup' || act === 'daily' || act === 'reward') {
        // ok
      } else if (HIDE[act]) {
        return;
      } else if (e.actor === 'robot' || e.actor === 'nyc' || e.actor === 'fairy' || e.actor === 'helper') {
        // log rời của worker → ẩn (đã có tổng hợp)
        if (act !== 'day_summary') return;
      }

      let text = e.summaryText || e.text || this.formatEventSummaryText(e) || e.action || 'Hành động';
      text = this.stripLogEmoji(text);
      let resultLine = e.resultLine || this.formatEventResultLine(e) || '';
      resultLine = this.stripLogEmoji(resultLine);
      const filter = e.filter || e.actor || this.resolveFilterKey(e.action, e.actor, text, e.category, e.mode);
      const firstAt = Number(e.firstAt) || ts;
      const lastAt = Number(e.lastAt) || ts;
      let timeText = e.timeText || '';
      if (!timeText) {
        if (e.aggregated && firstAt && lastAt && Math.abs(lastAt - firstAt) >= 60000) {
          timeText = this.formatLogClock(firstAt, false) + ' – ' + this.formatLogClock(lastAt, false);
        } else {
  ...
  async buyPlot(qty = 1) {
    ...
    const cost = price * qty;
    if (!this.chargeCoins(cost)) return { ok: false, msg: 'Không đủ tiền! Cần ' + cost + ' xu' };
    currentPlayer.stats.spent = (currentPlayer.stats.spent || 0) + cost;
