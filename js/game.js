...
const Game = {
  ...
  /**
   * Mỗi 3 giờ: hết hiệu lực tưới/phân.
   * Có Tiên + ô đang lớn:
   *   - Tưới lại đủ 3/3 (nhận đủ % nước: mỗi lần −12%, max −36%)
   *   - Có phân trong kho → bón (trừ 1 trong kho); không có thì thôi
   * Không Tiên: xóa nước/phân như cũ.
   */
  resetExpiredBoosts() {
    ...
    currentPlayer.plots.forEach(plot => {
      if (!plot) return;
      let waterExpired = (plot.waterCount || 0) > 0 && plot.lastWatered && (now - plot.lastWatered >= THREE_H);
      let fertExpired = !!(plot.fertilizerId && plot.fertilizedAt && (now - plot.fertilizedAt >= THREE_H));

      if (waterExpired || fertExpired) {
        if (fairy && plot.plantId && !this.isReady(plot)) {
          // Tưới đủ 3 lần → % nước full (−36% thời gian lớn)
          plot.waterCount = 3;
          plot.watered = true;
          ...
          // Phân: nếu còn trong kho thì bón (ưu tiên cùng loại cũ, không thì loại tốt nhất)
          if (!currentPlayer.inventory.fertilizers) currentPlayer.inventory.fertilizers = {};
          const bag = currentPlayer.inventory.fertilizers;
          let fertId = null;
          ...
          if (fertId) {
            ...
            // Hết phân trong kho → bỏ phân trên ô
            plot.fertilizerId = null;
            plot.fertilizedAt = null;
          }
          // Không trừ “ảo” thời gian: % tưới + % phân đã nằm trong getEffectiveGrowTime()
        } else {
          if (waterExpired) {
            plot.waterCount = 0;
            plot.watered = false;
      ...
      // Phân cũ không có fertilizedAt: gắn mốc khi phát hiện để bắt đầu đếm 3h từ lần load này
      if (plot.fertilizerId && !plot.fertilizedAt) {
        plot.fertilizedAt = now;
        changed = true;
      }
    });
    if (fairy && (wateredN > 0 || fertN > 0) && typeof this.addActivity === 'function') {
      this.addActivity(`🧚 Tiên chăm: tưới ${wateredN} ô` + (fertN ? `, bón ${fertN} ô` : ' (hết phân trong kho)'));
    }
    return changed;
  },
