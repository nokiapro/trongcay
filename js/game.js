...
const Game = {
  ...
  getBuffPrefs() {
    if (!currentPlayer) return { fairyEnabled: true, nycEnabled: true };
    ...
    return currentPlayer.buffPrefs;
  },

  setBuffPrefs(prefs) {
    if (!currentPlayer) return { ok: false, msg: 'Chưa đăng nhập!' };
    currentPlayer.buffPrefs = {
      fairyEnabled: !!(prefs && prefs.fairyEnabled),
      nycEnabled: !!(prefs && prefs.nycEnabled)
    };
    const a = [];
    a.push(currentPlayer.buffPrefs.fairyEnabled ? 'Tiên: bật' : 'Tiên: tắt');
    a.push(currentPlayer.buffPrefs.nycEnabled ? 'NYC: bật' : 'NYC: tắt');
    return { ok: true, msg: 'Đã lưu: ' + a.join(' · ') };
  },

  getNycConfig() {
    if (!currentPlayer) return { plantId: null, seedKind: 'normal', mode: 'all', count: 1 };
    if (!currentPlayer.nycConfig || typeof currentPlayer.nycConfig !== 'object') {
      currentPlayer.nycConfig = { plantId: null, seedKind: 'normal', mode: 'all', count: 1 };
    }
  ...
  async plantMultiple(plantId, count, preferredKind) {
    if (!currentPlayer) return { ok: false, msg: 'Chưa đăng nhập!' };
    const empty = [];
    ...
    let planted = 0;
    for (let i = 0; i < n; i++) {
      const res = await this.plantSeed(empty[i], plantId, preferredKind);
      if (res.ok) planted++;
      else break;
    }
  ...
  /**
   * Tiên đang active: tưới ngay mọi ô có cây mà chưa đủ 3/3 nước.
   * Không chờ chu kỳ 3 giờ.
   */
  fairyEnsureWatered() {
  ...
  /**
   * Mỗi tick:
   * - Có Tiên: tưới ngay ô chưa đủ nước; đủ 3 giờ kể từ lastFairyCare → chăm full (tưới + bón theo config)
  ...
  resetExpiredBoosts() {
    ...
    if (fairy) {
      // Luôn tưới các ô chưa đủ nước khi Tiên đang bật
      if (this.fairyEnsureWatered()) changed = true;
  ...
  /**
   * Người Yêu Cũ: khi có cây chín → thu hoạch ngay, rồi trồng lại theo cấu hình hạt.
   * mode=all → trồng hết ô trống (đến khi hết hạt)
   * mode=count → trồng tối đa `count` ô mỗi lần (sau thu / khi còn trống)
   * Không theo chu kỳ cố định — chạy ngay khi phát hiện cây chín hoặc ô trống cần trồng.
   */
  async runNycCare(now) {
    ...
    // 2) Trồng theo cấu hình (tách hạt thường / sao)
    ...
    if (cfg.plantId) {
      ...
      if (have > 0) {
        ...
        if (want > 0) {
          const res = await this.plantMultiple(cfg.plantId, want, kind);
          if (res.ok) {
  ...
  /** Có việc NYC cần làm không? (cây chín hoặc ô trống + còn hạt cấu hình) */
  nycHasWork() {
    if (!this.isNycActive() || !currentPlayer || !currentPlayer.plots) return false;
    for (const plot of currentPlayer.plots) {
      if (plot && plot.plantId && this.isReady(plot)) return true;
    }
    const cfg = this.getNycConfig();
  ...
  /** Tick NYC: khi có cây chín / ô trống cần trồng thì chạy ngay (không chờ 5 phút) */
  _nycBusy: false,
  async tickNycCare() {
    if (!this.isNycActive() || this._nycBusy) return false;
    if (!this.nycHasWork()) return false;
    this._nycBusy = true;
    try {
      return await this.runNycCare(Date.now());
    } finally {
      this._nycBusy = false;
    }
  },
